...
class Browser extends CI_Controller {
	...
	public function index() //the default function called when the page is first visited
	{
		...
		$result = $this->db->query('SELECT * FROM `invmarketgroups` WHERE `parentGroupID` IS NULL ORDER BY `invmarketgroups`.`marketGroupName` ASC'); // query the database to retrieve the initial base market catagories
		while ($row = mysql_fetch_array($result) //for each market catagory extract the data and put it into the form of a php array
		{
			array_push($data['result'],$row); // push that array of data onto a slot at the end of this array
		}
